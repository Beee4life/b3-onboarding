            ...
            <?php if ( ! is_multisite() && 'none' != $registration_type ) { ?>
                ...
                <?php if ( ! $email_only ) { ?>
                    ...
                    <?php b3_get_settings_field_open( $hide_username_restrictions, 'username-restrictions' ); ?>
                        ...
                        <?php b3_get_close(); ?>
                        <div class="b3_settings-input b3_settings-input--text">
                            <div class=""><?php echo sprintf( esc_html__( 'Some usernames are excluded already by default, see them %s.', 'b3-onboarding' ), sprintf( '<a href="%s">%s</a>', sprintf( '%s/function/b3_get_default_reserved_user_names/', B3OB_PLUGIN_SITE ), esc_html__( 'here', 'b3-onboarding' ) ) ); ?></div>
